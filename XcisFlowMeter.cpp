...
void XcisFlowMeter::initialise()
{
    Serial.println("XcisFlowMeter::initialise");
...
void XcisFlowMeter::processMessage(uint8_t *data , uint8_t *responseData)
{
    uint8_t recvPayload[28];
    //uint8_t responseData[32];

    Serial.print("XcisFlowMeter::processMessage:");
    ...
    { 
      ...
      xcisMessage.createMessage(responseData,xcisMessage.getLocationID(), FLOW_METER, SENSOR_DATA_RESPONSE);
  
      Serial.print("Response:");
      xcisMessage.dumpHex(responseData,XCIS_RH_MESH_MAX_MESSAGE_LEN);
    }
}
void XcisFlowMeter::countPulses()
{
    pulseCount++;
}
void XcisFlowMeter::storePulseCount()
{
